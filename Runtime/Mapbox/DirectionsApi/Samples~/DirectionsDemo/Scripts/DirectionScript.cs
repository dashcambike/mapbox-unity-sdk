					...
					Feature = new VectorFeatureUnity
					{
						Points = new List<List<Vector3>>()
						{
							route.Geometry.Select(geo =>
								_mapInformation.ConvertLatLngToPosition(new LatitudeLongitude(geo.x, geo.y))).ToList()
						}
					}
				};
	...
	private class LineMeshCore
	{
			...
			foreach (var roadSegment in allPoints)
			{
				...
				for (int i = 0; i < roadSegmentCount; i++)
				{
					...
					else if (currentJoin == JoinType.Bevel || currentJoin == JoinType.Fakeround)
					{
						...
						if (!_startOfLine)
						{
							AddCurrentVertex(_currentVertex, _distance, _prevNormal, md, _scaledWidth, _cornerOffsetA,
								_cornerOffsetB);
						}
						...
						if (currentJoin == JoinType.Fakeround)
						{
							...
							for (var m = 0f; m < n; m++)
							{
								...
								AddPieSliceVertex(_currentVertex, _distance, approxFractionalJoinNormal, lineTurnsLeft,
									md, _scaledWidth);
							}
						...
						{
							AddCurrentVertex(_currentVertex, _distance, _nextNormal, md, _scaledWidth, -_cornerOffsetA,
								-_cornerOffsetB);
						}
					}
		...
		private bool IsOnEdge(Vector3 p1, Vector3 p2, float tolerance)
		{
			return ((Math.Abs(Math.Abs(p1.x) - (_tileSize / 2)) < tolerance &&
			         Math.Abs(Math.Abs(p2.x) - (_tileSize / 2)) < tolerance && Math.Sign(p1.x) == Math.Sign(p2.x)) ||
